#!/usr/bin/env python3
"""Full cognition-first orchestrator for evez-agentnet.
...
log = logging.getLogger("agentnet-cognition-full")
...
def main():
    ...
    state["round"] += 1
    rnd = state["round"]
    append_spine("round_start", {"round": rnd, "mode": "cognition_full"})
    ...
    _DAEMON.state.self_model["predictor_entropy"] = uncertainty.get("entropy", 0.0)
    ...
    if uncertainty.get("entropy", 0.0) >= 1.2:
        _DAEMON.state.unresolved_residue.append(f"predictor_entropy:{uncertainty['entropy']}")
        _DAEMON.state.self_model["action_mode"] = "evidence_seek"
        append_spine("cognition_gate", {"round": rnd, "gate": "predictor_entropy", "entropy": uncertainty.get("entropy", 0.0)})
    ...
    if action_mode != "evidence_seek":
        ...
        append_spine("generate_complete", {"round": rnd, "count": len(drafts), "action_mode": action_mode})
...
if __name__ == "__main__":
    while True:
        ...
        log.info("Sleeping %ss until next cognition-full round...", interval)
        time.sleep(interval)
